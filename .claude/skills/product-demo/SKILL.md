...
## 3. Relay for the live agent beats (the CFO question)

The Monica-asks beat needs a relay the page reaches over **wss — never
`ws://127.0.0.1`** (loopback is not how the product is demoed, and Chrome
blocks/usually prompts on loopback WS from a public https page). Two
sanctioned endpoints:

- **Tailscale (on-prem)** — on the demo host: `tailscale serve --bg --https=443
  http://127.0.0.1:<relay-port>`, then `?sync=wss://<host>.<tailnet>.ts.net` in
  the story. Run `sync serve --addr 127.0.0.1:<relay-port>` and
  `sync agent --principal cfo --watch-doc finops --addr 127.0.0.1:<relay-port>`
  from the **current build** (`cargo build -p sync`).
- **Hosted ECS** — `infra/ecs` (`pnpm outputs` → `relayUrl`); the task co-hosts
  the editor agent. Redeploy (`pnpm up`) first if the image predates the
  feature being demoed.

Before recording, reset the room: `rm ~/.contextful/docs/finops.loro`, restart
serve + agent, and make sure **no stray browser tab** holds a persisted
`?sync=` override pointing at this relay — old tabs replay stale CRDT history
(including past demo asks), which makes the question pre-exist so the live
typing is skipped and the answer wait times out. Navigate such tabs to
`?sync=off`.
...
## 5. Editing or authoring stories
...
- Never point `?sync=` at `ws://127.0.0.1:…` in a committed story — use the Tailscale `wss://….ts.net` proxy or the hosted ECS relay (§3).
