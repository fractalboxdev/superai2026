...
//
// VITE_* values are inlined at build time, so the key must be set in the Vercel
// project's environment before building (see OBSERVABILITY.md).
  ...
  useEffect(() => {
    ...
    void import("@hyperdx/browser").then(({ default: HyperDX }) => {
      ...
      HyperDX.init({
        apiKey,
        service:
          import.meta.env.VITE_OTEL_SERVICE_NAME ?? "contextful-web-browser",
