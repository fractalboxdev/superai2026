...
- **Halftone-teal comic** (HBO *Silicon Valley* homage, per the repo-root PRESENTATION.md theme rule): retro comic-book ink illustrations with bold black outlines and halftone dot shading. Palette: deep teal green (`#14534a`/`#2f9e77`, the `teal` primitives in `packages/design-system/tokens.json`) dominant, indigo (`#4f46e5`) secondary, amber (`#f59e0b`) as a small accent. The shared style prompt lives in `apps/landing/arts.manifest.mjs`; keep it in sync with the design system.
- The hand-drawn comic frames and cast portraits in the repo-root `assets/` are the style reference. Cast portraits ship pre-staged at `public/cast/<slug>.webp` (Pied Piper team, displayed **"Name (Role)"**) — they are not regenerated by the pipeline.
- Generated illustrations keep a **near-white empty background** even in the comic style — the pipeline knocks the background out to a transparent silhouette; halftone texture lives on the subject, never the background.
- Comic frames, cast portraits, and the demo video are framed with the design-system `.cf-comic` panel (thick ink border + hard offset shadow + deep-teal backdrop); `.cf-halftone` adds the dot texture to section backdrops.
